...
double GetMaxFrequency(const std::vector<uint8_t> &data, double sampling_frequency = 16000, double amplitude = 150)
{
    std::vector<double> vReal;
    std::vector<double> vImag;
    for (uint16_t i = 0; i < data.size(); i += 2)
    {
        uint16_t buffer = (data[i] << 8);
        buffer += data[i + 1];
        vReal.push_back((double)buffer);
        vImag.push_back(0.0);
    }
