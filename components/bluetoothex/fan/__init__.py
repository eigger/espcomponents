...
from esphome.components import fan, bluetoothex
from esphome.const import CONF_OUTPUT_ID, \
    CONF_STATE, CONF_COMMAND, CONF_UPDATE_INTERVAL
from .. import bluetoothex_ns, cmd_t, uint8_ptr_const, uint16_const
from ..const import CONF_SPEED_CNT, CONF_STATE_SPEED, CONF_COMMAND_SPEED

DEPENDENCIES = ['bluetoothex']
BluetoothExFan = bluetoothex_ns.class_('BluetoothExFan', cg.Component)

CONFIG_SCHEMA = cv.All(fan.FAN_SCHEMA.extend({
    cv.GenerateID(CONF_OUTPUT_ID): cv.declare_id(BluetoothExFan),
