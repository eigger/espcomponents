...
from esphome.components import sensor, bluetoothex
...
from ..const import CONF_BLUETOOTHEX_ID, CONF_SUB_FILTER, CONF_FILTER, CONF_STATE_NUMBER, CONF_COMMAND_UPDATE, CONF_LENGTH, CONF_PRECISION

DEPENDENCIES = ['bluetoothex']
BluetoothExSensor = bluetoothex_ns.class_(
    'BluetoothExSensor', sensor.Sensor, cg.PollingComponent)

CONFIG_SCHEMA = cv.All(sensor.SENSOR_SCHEMA.extend({
    cv.GenerateID(): cv.declare_id(BluetoothExSensor),
