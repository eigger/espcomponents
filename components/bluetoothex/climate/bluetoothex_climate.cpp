#include "bluetoothex_climate.h"
#include "esphome/core/log.h"

namespace esphome {
namespace bluetoothex {

static const char *TAG = "bluetoothex.climate";

void BluetoothExClimate::dump_config()
{
    ESP_LOGCONFIG(TAG, "BluetoothEx Climate '%s':", device_name_->c_str());
    dump_bluetoothex_device_config(TAG);
}

climate::ClimateTraits BluetoothExClimate::traits()
{
...
void BluetoothExClimate::publish(const std::vector<uint8_t>& data)
{
    bool changed = false;
    // turn off
...
void BluetoothExClimate::control(const climate::ClimateCall &call)
{
    // Set mode
