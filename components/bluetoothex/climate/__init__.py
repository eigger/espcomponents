...
from esphome.components import climate, bluetoothex, sensor
from esphome.const import CONF_ID, CONF_SENSOR, CONF_OFFSET
from .. import bluetoothex_ns, command_hex_schema, STATE_NUM_SCHEMA, cmd_t, uint8_ptr_const, uint16_const, \
...
BluetoothExClimate = bluetoothex_ns.class_('BluetoothExClimate', climate.Climate, cg.Component)

CONFIG_SCHEMA = cv.All(climate.CLIMATE_SCHEMA.extend({
    cv.GenerateID(): cv.declare_id(BluetoothExClimate),
    ...
    cv.Optional(CONF_COMMAND_ON): cv.invalid("BluetoothEx Climate do not support command_on!"),
    cv.Optional(CONF_STATE_ON): cv.invalid("BluetoothEx Climate do not support state_on!")
