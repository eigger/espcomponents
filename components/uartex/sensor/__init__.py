...
CONFIG_SCHEMA = cv.All(sensor.SENSOR_SCHEMA.extend({
    ...
    cv.Optional(CONF_STATE_NUM): STATE_NUM_SCHEMA
}).extend(cv.polling_component_schema('60s')), cv.has_exactly_one_key(CONF_LAMBDA, CONF_STATE_NUM))
...
def to_code(config):
    ...
    if CONF_STATE_NUM in config:
        data = config[CONF_STATE_NUM]
