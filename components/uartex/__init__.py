...
async def to_code(config):
    ...
    if CONF_ON_WRITE in config:
        templ = await cg.templatable(config[CONF_ON_WRITE], [(uint8_ptr_const, 'data'), (uint16_const, 'len')], cg.void)
        cg.add(var.set_on_write(templ))
    if CONF_ON_READ in config:
        templ = await cg.templatable(config[CONF_ON_READ], [(uint8_ptr_const, 'data'), (uint16_const, 'len')], cg.void)
