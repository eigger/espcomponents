...
void UARTExComponent::dump_config() {
  ...
  if (this->conf_rx_data_length_.has_value()) {
    ...
    ESP_LOGD(TAG,
             "  rx_data_length: offset=%d, length=%d, endian=%s, adjust=%d",
             dl.offset, dl.length, dl.endian == ENDIAN_BIG ? "big" : "little",
             dl.adjust);
  }
  ...
  if (this->rx_header_.has_value())
    log_config(TAG, "rx_header", this->rx_header_.value().data);
  if (this->rx_header_.has_value())
    log_config(TAG, "rx_header mask", this->rx_header_.value().mask);
  if (this->rx_footer_.has_value())
    log_config(TAG, "rx_footer", this->rx_footer_.value());
  if (this->tx_header_.has_value())
    log_config(TAG, "tx_header", this->tx_header_.value());
  if (this->tx_footer_.has_value())
    log_config(TAG, "tx_footer", this->tx_footer_.value());
  ...
  if (this->tx_ctrl_pin_)
    LOG_PIN("tx_ctrl_pin: ", this->tx_ctrl_pin_);
#endif
}
...
void UARTExComponent::setup() {
  ...
  if (this->rx_checksum_ != CHECKSUM_NONE)
    this->rx_parser_.set_checksum_len(1);
  ...
  if (this->rx_footer_.has_value())
    this->rx_parser_.add_footers(this->rx_footer_.value());
  this->rx_parser_.set_total_len(this->conf_rx_length_);
  if (this->conf_rx_data_length_.has_value()) {
    auto &dl = this->conf_rx_data_length_.value();
    this->rx_parser_.set_data_length(dl.offset, dl.length,
                                     dl.endian == ENDIAN_BIG, dl.adjust);
  }
  this->rx_parser_.set_buffer_len(this->parent_->get_rx_buffer_size());
  if (this->error_)
    this->error_->publish_state("None");
  if (this->version_)
    this->version_->publish_state(UARTEX_VERSION);
...
bool UARTExComponent::read_from_uart() {
  if (this->rx_priority_ == PRIORITY_DATA) {
    ...
    if (!this->rx_processing_ ||
        (!this->available() &&
         elapsed_time(this->rx_timer_) > this->conf_rx_timeout_)) {
      this->rx_processing_ = false;
      this->rx_parser_.clear();
      this->rx_timer_ = get_time();
    }
...
bool UARTExComponent::verify_ack() {
  if (!is_tx_cmd_pending())
    return false;
  if (!equal(this->rx_parser_.data(current_tx_cmd()->mask),
             current_tx_cmd()->ack))
    return false;
  tx_cmd_result(true);
  ESP_LOGD(TAG, "Ack: %s, Gap Time: %lums",
           to_hex_string(this->rx_parser_.buffer()).c_str(),
           elapsed_time(this->tx_time_));
  return true;
}
...
void UARTExComponent::publish_data() {
  ...
  if (!buf.empty())
    this->read_callback_.call(buf.data(), buf.size());
  this->match_callback_.call(data, nullptr);
  publish_rx_log(buf);
  for (UARTExDevice *device : this->devices_) {
    device->parse_data(data);
  }
#ifdef ESPHOME_LOG_HAS_VERBOSE
  ESP_LOGV(TAG, "Receive data-> %s",
           to_hex_string(this->rx_parser_.buffer()).c_str());
#endif
}
...
void UARTExComponent::dequeue_tx_data_from_devices() {
  for (UARTExDevice *device : this->devices_) {
    ...
    if (cmd_low_priority != nullptr)
      enqueue_tx_data({device, cmd_low_priority}, true);
  }
}

void UARTExComponent::write_to_uart() {
  if (elapsed_time(this->rx_time_) < this->conf_tx_delay_)
    return;
  if (elapsed_time(this->tx_time_) < this->conf_tx_delay_)
    return;
  if (is_tx_cmd_pending() &&
      elapsed_time(this->tx_time_) < this->conf_tx_timeout_)
    return;
  if (retry_tx_data())
    return;
  write_tx_data();
...
void UARTExComponent::write_tx_cmd() {
  ...
  if (this->tx_ctrl_pin_)
    this->tx_ctrl_pin_->digital_write(true);
  if (this->tx_header_.has_value()) {
    command.insert(command.end(), this->tx_header_.value().begin(),
                   this->tx_header_.value().end());
  }
  command.insert(command.end(), current_tx_cmd()->data.begin(),
                 current_tx_cmd()->data.end());
  if (this->tx_checksum_ != CHECKSUM_NONE ||
      this->tx_checksum_2_ != CHECKSUM_NONE) {
    std::vector<uint8_t> checksum = get_tx_checksum(current_tx_cmd()->data);
    command.insert(command.end(), checksum.begin(), checksum.end());
  }
  if (this->tx_footer_.has_value()) {
    command.insert(command.end(), this->tx_footer_.value().begin(),
                   this->tx_footer_.value().end());
  }
  write_data(command);
  write_flush();
  if (this->tx_ctrl_pin_)
  ...
  this->tx_retry_cnt_++;
  this->tx_time_ = get_time();
  if (current_tx_cmd()->ack.empty())
    tx_cmd_result(true);
  if (!command.empty())
    this->write_callback_.call(command.data(), command.size());
...
void UARTExComponent::enqueue_tx_data(const tx_data_t data, bool low_priority) {
  if (low_priority)
    this->tx_queue_low_priority_.push(data);
  else
...
void UARTExComponent::write_command(cmd_t cmd) {
  ...
  if (++tx_command_cnt_ >= conf_tx_command_queue_size_)
    tx_command_cnt_ = 0;
}
...
void UARTExComponent::set_rx_data_length(uint8_t offset, uint8_t length,
                                         ENDIAN endian, int8_t adjust) {
...
void UARTExComponent::tx_cmd_result(bool result) { clear_tx_data(); }
...
ERROR UARTExComponent::validate_data() {
  ...
  if (this->conf_rx_length_ > 0 &&
      this->conf_rx_length_ != this->rx_parser_.buffer().size()) {
    return ERROR_SIZE;
  }
  if (this->rx_header_.has_value() &&
      this->rx_parser_.parse_header() == false) {
    return ERROR_HEADER;
  }
  if (this->rx_footer_.has_value() &&
      this->rx_parser_.parse_footer() == false) {
    return ERROR_FOOTER;
  }
  if ((this->rx_checksum_ != CHECKSUM_NONE ||
       this->rx_checksum_2_ != CHECKSUM_NONE) &&
      !this->rx_parser_.verify_checksum(
          get_rx_checksum(data, this->rx_parser_.header()))) {
    return ERROR_CHECKSUM;
  }
  if (!this->rx_footer_.has_value() && this->conf_rx_length_ == 0 &&
      this->rx_checksum_ == CHECKSUM_NONE &&
      this->rx_checksum_2_ == CHECKSUM_NONE) {
...
bool UARTExComponent::publish_error(ERROR error_code) {
  ...
  switch (error_code) {
  case ERROR_SIZE:
    ESP_LOGW(TAG, "[Read] Size error: %s",
             to_hex_string(this->rx_parser_.buffer()).c_str());
    if (this->error_ && this->error_code_ != ERROR_SIZE)
      this->error_->publish_state("Size Error");
    break;
  case ERROR_HEADER:
    ESP_LOGW(TAG, "[Read] Header error: %s",
             to_hex_string(this->rx_parser_.buffer()).c_str());
    if (this->error_ && this->error_code_ != ERROR_HEADER)
      this->error_->publish_state("Header Error");
    break;
  case ERROR_FOOTER:
    ESP_LOGW(TAG, "[Read] Footer error: %s",
             to_hex_string(this->rx_parser_.buffer()).c_str());
    if (this->error_ && this->error_code_ != ERROR_FOOTER)
      this->error_->publish_state("Footer Error");
    break;
  case ERROR_CHECKSUM:
    ESP_LOGW(TAG, "[Read] Checksum error: %s",
             to_hex_string(this->rx_parser_.buffer()).c_str());
    if (this->error_ && this->error_code_ != ERROR_CHECKSUM)
      this->error_->publish_state("Checksum Error");
    break;
  case ERROR_RX_TIMEOUT:
    ESP_LOGW(TAG, "[Read] Rx Timeout error: %s",
             to_hex_string(this->rx_parser_.buffer()).c_str());
    if (this->error_ && this->error_code_ != ERROR_RX_TIMEOUT)
      this->error_->publish_state("Rx Timeout Error");
    break;
  case ERROR_TX_TIMEOUT:
    ESP_LOGW(TAG, "[Read] Tx Timeout error: %s",
             to_hex_string(this->rx_parser_.buffer()).c_str());
    if (this->error_ && this->error_code_ != ERROR_TX_TIMEOUT)
      this->error_->publish_state("Tx Timeout Error");
    break;
  case ERROR_NONE:
    if (this->error_ && this->error_code_ != ERROR_NONE)
      this->error_->publish_state("None");
    error = false;
    break;
  }
...
void UARTExComponent::publish_rx_log(const std::vector<unsigned char> &data) {
  if (this->log_ == nullptr)
    return;
  if (this->log_ascii_)
    publish_log("[R]" + to_ascii_string(data));
  else
    publish_log("[R]" + to_hex_string(data));
}
...
void UARTExComponent::publish_tx_log(const std::vector<unsigned char> &data) {
  if (this->log_ == nullptr)
    return;
  if (this->log_ascii_)
    publish_log("[W]" + to_ascii_string(data));
  else
    publish_log("[W]" + to_hex_string(data));
}
...
void UARTExComponent::set_rx_checksum(
    std::function<uint8_t(const uint8_t *data, const uint16_t len)> &&f) {
...
void UARTExComponent::set_tx_checksum(
    std::function<uint8_t(const uint8_t *data, const uint16_t len)> &&f) {
...
void UARTExComponent::set_rx_checksum_2(
    std::function<std::vector<uint8_t>(const uint8_t *data, const uint16_t len)>
        &&f) {
...
void UARTExComponent::set_tx_checksum_2(
    std::function<std::vector<uint8_t>(const uint8_t *data, const uint16_t len)>
        &&f) {
...
std::vector<uint8_t>
UARTExComponent::get_rx_checksum(const std::vector<uint8_t> &data,
...
std::vector<uint8_t>
UARTExComponent::get_tx_checksum(const std::vector<uint8_t> &data) {
  if (this->tx_checksum_f_.has_value()) {
    ...
    std::vector<uint8_t> header =
        this->tx_header_.value_or(std::vector<uint8_t>{});
    if (this->tx_checksum_ != CHECKSUM_NONE) {
...
uint16_t UARTExComponent::get_checksum(CHECKSUM checksum,
                                       const std::vector<uint8_t> &header,
