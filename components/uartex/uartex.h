...
class UartExComponent;
class UartExDevice;
class UartExListener;
...
public:
    ...
    void set_parent(UartExComponent *parent) { parent_ = parent; }

    void set_monitor(bool monitor) { monitor_ = monitor; }
    bool is_monitor() { return monitor_; }

protected:
    UartExComponent *parent_{nullptr};
    bool monitor_{false};
};
 ...
 * UartEx Device
 */
class UartExDevice : public UartExListener, public PollingComponent
{
public:
 ...
 */
class UartExComponent : public Component
{
public:
    UartExComponent(int baud, num_t data = 8, num_t parity = 0, num_t stop = 1, num_t rx_wait = 15)
    {
        conf_baud_ = baud;
        conf_data_ = data;
        conf_parity_ = parity;
    ...
    void set_prefix(std::vector<uint8_t> prefix)
    {
        prefix_ = prefix;
    ...
    void set_suffix(std::vector<uint8_t> suffix)
    {
        suffix_ = suffix;
        suffix_len_ = suffix.size();
    }

    /** CheckSum8 Xor 사용 여부 (수신시 Check, 발신시 Append) */
    void set_checksum(bool checksum) { checksum_ = checksum; } // xor sum
    void set_checksum_lambda(std::function<uint8_t(const uint8_t *data, const num_t len)> &&f)
    {
        checksum_f_ = f;
        checksum_ = true;
    }
    ...
    void register_listener(UartExListener *listener)
    {
...
protected:
    ...
    UartExDevice *tx_current_device_{nullptr};
...
/** 패킷 캡쳐용 리스너 */
class SerialMonitor : public UartExListener
{
public:
