...
enum PRIORITY { PRIORITY_DATA, PRIORITY_LOOP };
...
public:
  ...
  void set_tx_checksum(
      std::function<uint8_t(const uint8_t *data, const uint16_t len)> &&f);
  void set_rx_checksum_2(CHECKSUM checksum);
  void set_tx_checksum_2(CHECKSUM checksum);
  void set_rx_checksum_2(std::function<std::vector<uint8_t>(
                             const uint8_t *data, const uint16_t len)> &&f);
  void set_tx_checksum_2(std::function<std::vector<uint8_t>(
                             const uint8_t *data, const uint16_t len)> &&f);
  ...
  void add_on_write_callback(
      std::function<void(const uint8_t *data, const uint16_t len)> &&callback) {
    this->write_callback_.add(std::move(callback));
  }
  void add_on_read_callback(
      std::function<void(const uint8_t *data, const uint16_t len)> &&callback) {
    this->read_callback_.add(std::move(callback));
  }
  void add_on_error_callback(std::function<void(const ERROR)> &&callback) {
    this->error_callback_.add(std::move(callback));
  }
  void add_on_match_callback(std::function<void(const std::vector<uint8_t> &,
                                                const state_t *)> &&callback) {
    this->match_callback_.add(std::move(callback));
  }
  ...
  void set_rx_data_length(uint8_t offset, uint8_t length, ENDIAN endian,
                          int8_t adjust);
...
protected:
  ...
  uint16_t get_checksum(CHECKSUM checksum, const std::vector<uint8_t> &header,
...
protected:
  ...
  optional<std::function<uint8_t(const uint8_t *data, const uint16_t len)>>
      rx_checksum_f_{};
  optional<std::function<uint8_t(const uint8_t *data, const uint16_t len)>>
      tx_checksum_f_{};
  CHECKSUM rx_checksum_2_{CHECKSUM_NONE};
  CHECKSUM tx_checksum_2_{CHECKSUM_NONE};
  optional<std::function<std::vector<uint8_t>(const uint8_t *data,
                                              const uint16_t len)>>
      rx_checksum_f_2_{};
  optional<std::function<std::vector<uint8_t>(const uint8_t *data,
                                              const uint16_t len)>>
      tx_checksum_f_2_{};
  CallbackManager<void(const uint8_t *data, const uint16_t len)>
      write_callback_{};
  CallbackManager<void(const uint8_t *data, const uint16_t len)>
      read_callback_{};
  ERROR error_code_{ERROR_NONE};
  CallbackManager<void(const ERROR)> error_callback_{};
  CallbackManager<void(const std::vector<uint8_t> &, const state_t *)>
      match_callback_{};
