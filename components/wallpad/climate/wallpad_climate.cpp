...
static const char *TAG = "wallpad.climate";

void WallPadClimate::dump_config()
{
    ESP_LOGCONFIG(TAG, "WallPad Climate '%s':", device_name_->c_str());
...
void WallPadClimate::control(const climate::ClimateCall &call)
{
    ...
    if (call.get_target_temperature().has_value() && this->target_temperature != *call.get_target_temperature())
    {
        ...
        send_command(&this->command_temperature_);
    }
    ...
    if (this->command_away_.has_value() && call.get_preset().has_value() && *preset != *call.get_preset())
    {
        *preset = *call.get_preset();
        ...
        {
            send_command(&this->command_away_.value());
        }
        else if (this->command_home_.has_value())
        {
            send_command(&this->command_home_.value());
        }
