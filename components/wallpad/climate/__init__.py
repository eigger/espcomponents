...
from esphome.components import climate, wallpad, sensor
from esphome.const import CONF_ID, CONF_SENSOR, CONF_OFFSET
from .. import wallpad_ns, command_hex_schema, STATE_NUM_SCHEMA, cmd_hex_t, uint8_ptr_const, num_t_const, \
...
wallpadClimate = wallpad_ns.class_(
    'WallPadClimate', climate.Climate, cg.Component)

CONFIG_SCHEMA = cv.All(climate.CLIMATE_SCHEMA.extend({
    cv.GenerateID(): cv.declare_id(wallpadClimate),
    ...
    cv.Optional(CONF_COMMAND_ON): cv.invalid("WallPad Climate do not support command_on!"),
    cv.Optional(CONF_STATE_ON): cv.invalid("WallPad Climate do not support state_on!")
